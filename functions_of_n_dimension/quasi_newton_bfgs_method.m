...
function [step_size1,point, output, time, iter] = quasi_newton_bfgs_method(f, f_gradient, point, epsilon, max_iter)
...
%step_size=0.1;
    iter=1;
while norm(gradient)>epsilon && iter<max_iter
    if (iter == max_iter)
    ...
    %step_size=0.602764;
    step_size = armijo(f, point, step_size, gradient, -(Hk * gradient)', max_iter);

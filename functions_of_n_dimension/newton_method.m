...
before = f(point(1), point(2)); % initial function value
   ...
   iter=1;
%d=double((-inv(f_hessian(point(1), point(2)))))*double(f_gradient(point(1), point(2)));
%d = (inv(f_hessian(point(1), point(2))) * gradient_mat')';
%  hessian_mat1 = double(f_hessian(point(1), point(2)));
%  gradient_mat1 = double(f_gradient(point(1), point(2)))';
% d0= (-inv(hessian_mat1) * gradient_mat1')';
...
for iter=1:max_iter
    ...
    if (isempty(find(diag(eig_diagnol) <= 0)) == 1)
        ...
        %step_size = 0.01; % step_size initialization
        step_size = armijo(f, point, step_size, gradient_mat, -gradient_mat, max_iter);
